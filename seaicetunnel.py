...
dates_without_year = [
    datetime.date(1980, x[0].month, x[0].day) for x in data.index
]
...
days_from_1jan = [
    (x[0].date() - datetime.date(x[0].year, 1, 1)).days for x in data.index
]
thetas = numpy.array([float(x) / 365 * 2 * numpy.pi for x in days_from_1jan])
